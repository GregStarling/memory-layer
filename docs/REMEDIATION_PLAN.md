...
> **STATUS: IN PROGRESS (committed as WIP 2026-07-08, 10 tests failing — DO NOT RELEASE from this commit).** Phase 3 implementation is complete; the adversarial-review fix round is ~half done. DONE: shared kernel, rank/ordering/filter parity, pg governance, and the fix-round's CoreLeaks (core cross-scope leak gates in manager + temporal replay, `created_at` contract, memory) + ConformanceCI (leak-detecting conformance assertions, int4 cap, pg CI wiring). NOT DONE: the SQLite + Postgres adapter fixers (apply visibility gates to adapters' cross-scope + semantic + event-log reads; pg insertPlaybook visibility_class; pg `to_tsquery('')` empty guard; route all pg inserts' `created_at` through int4 coercion; ordering alignment). The 10 failures are all SQLite cross-scope/collaboration tests — expected red regression guards, not defects. TO RESUME: `Workflow({scriptPath: ".claude/workflows/phase3-fixes.mjs", resumeFromRunId: "wf_87e7644e-b74"})` (CoreLeaks + ConformanceCI replay from cache; adapter fixers run fresh), then gate chain → commit → push → chase CI (the postgres-integration job now runs the conformance suite against real pg for the first time — the true gate for the pg fixes). Full detail in the maintainer's session memory. Original staging note below.
>
