...
> **STATUS: IMPLEMENTED 2026-07-07** (uncommitted → committed). Items 2.1–2.5 done with a seeded replay-equivalence property test (the release gate) + event-integrity + cursor conformance suites, all parameterized over the three adapters. Two rounds: implementation (7-agent workflow) + a fix round that resolved everything three adversarial reviewers found. Reviewers caught real defects: a playbook-revision replay divergence (now fold `playbook.updated`), pg HNSW built but never used (query now matches the index expression), a pg advisory lock leaking across pooled connections, the default embedding filter killing semantic search, and handoff list reads still writing-on-read (added `expireStaleHandoffs` reaper). Manager decisions locked: event_id-only ordering, lazy partial per-dimension HNSW, filter model only when known, metadata-aware reembed. Deferred to later phases (documented, not silent): pg input.created_at still dropped on domain inserts (3.5); single-watermark-row serialization (post-4.3.0 follow-up); handoff batch-atomicity parity nuances (3.10). Full suite 1021 passed; lint/build/coverage/eval gates green locally.
...
> **STATUS: COMPLETE — COMMITTED + CI-GREEN on origin/main 2026-07-13** (commits 9504142 WIP checkpoint → 612ad96 fix round → 96f45e1 pgvector harness fix; CI run 29262787724, all 5 jobs green). The postgres-integration job now runs the adapter-conformance + pg parity suites against real Postgres — the first live-pg verification of every cross-scope visibility gate. Items 3.1–3.10 done with tests. Notable additions beyond the plan: `learnFact` gained an optional `visibilityClass` (mirroring `trackWorkItem`), threaded through MCP `memory_learn_fact` + HTTP `POST /v1/facts` — required because the F4 visibility gates correctly block private facts cross-scope, and the learn write-path could previously only produce private knowledge. CI round-trip caught one pg-only bug (test harness, not adapter SQL): `CREATE EXTENSION vector` on a search_path'd pool installed pgvector into the ephemeral test schema and teardown CASCADE dropped it for parallel workers — fixed with a shared `ensurePgVectorExtension` pinned to `public`. Deferred (documented): Y2038 INTEGER→BIGINT `created_at`; integer coercion for non-`created_at` INTEGER binds. NOTE: no per-phase npm publishes happened; the consolidated 4.4.0 version bump landed with Phase 4 (publish itself remains manual). Historical in-progress note below.
>
> **STATUS: STAGED 2026-07-07** — executable workflow at `.claude/workflows/phase3-adapter-parity.mjs`. Launch with `Workflow({scriptPath: ".claude/workflows/phase3-adapter-parity.mjs"})`. Four stages: Kernel (extract src/adapters/shared/, scope-model docs, memory reference) → SQLite+Postgres in parallel → Conformance suite (the release gate) → 3 adversarial reviewers. Manager decisions pre-made in the workflow (P1 search-contract = result-SET equality for exact-term queries + identical options/ordering, NOT identical multi-term ranking; P2 rank→(0,1]; P3 one ordering per method; P4 filters-before-LIMIT; P5 pg honors caller created_at + persists all fields; P6 visibility end-to-end + sqlite schema v22; P7 no new config; P8 sync/async triple-sync). Phase-2 lesson baked in: pg-only bind/type bugs are invisible locally, so the conformance suite must run in the postgres-integration CI job and all pg SQL gets hand-verified. Same close-out as prior phases: triage reviews → Opus fix round → local gate chain → commit (NOT README) → fetch-then-push → chase CI (esp. postgres-integration) green.
...
> **STATUS: COMPLETE — COMMITTED + CI-GREEN on origin/main 2026-07-14** (commit a20fe62; CI run 29339289744, all 5 jobs green). Items 4.1–4.6 done: the five phantom paths implemented as real routes (4.5's implement-not-delete), five undocumented routes specced, bidirectional spec⇆route parity test + YAML-parser-integrity self-test + enum-membership validation; README claims corrected against code with reviewer verification; examples fixed and typechecked in CI; presets ship real token budgets (8k/4k/6k) with `UNLIMITED_TOKEN_BUDGET` exported as the explicit opt-in; `.npmignore` deleted; version bumped 4.1.0 → 4.4.0 (npm publish remains manual — 4.4.0 is NOT yet on npm). Adversarial review caught a real parallel-worker seam (README documented a `tokenBudget: 'unlimited'` string API the implementation deliberately didn't build) — fixed, plus the un-exported `UNLIMITED_TOKEN_BUDGET` constant the reviewers missed. Remaining 4.6 externals for a maintainer: PyPI old-name deprecation check, stale python dist artifacts cleanup. Original staging note below.
>
> **STATUS: STAGED 2026-07-13 (superseded)** — executable workflow at `.claude/workflows/phase4-honest-surfaces.mjs`. Launch with `Workflow({scriptPath: ".claude/workflows/phase4-honest-surfaces.mjs"})`. Two stages: 4 parallel Opus workers on disjoint file groups (ApiTruth = five wiki routes + bidirectional spec↔route parity test; DocsTruth = README/docs claims verified against code; ExamplesCI = example fixes + examples-typecheck CI step + packaging hygiene + the consolidated 4.1.0→4.4.0 version bump; Budgets = real preset token budgets with trim-trace) → 2 adversarial reviewers (API/e2e truth; every-doc-claim-vs-code). Manager decisions pre-baked (D1 implement-not-delete the five routes as thin dispatches through the existing auth/scope pipeline; D2 spec follows code where shapes disagree; D3 version bump in-phase, npm publish stays manual; D4 budgets ai_ide 8k / chat_agent 4k / autonomous_agent 6k with explicit unlimited opt-in; D5 honesty edits state facts, not apologies; D6 delete .npmignore, keep "files"). Scouted facts baked into prompts: all five phantom endpoints have existing backing code (ingestDocument, promoteResponse, exportAsMarkdown at manager.ts:3298, lintKnowledge in core/knowledge-lint, refreshDocuments); example bugs confirmed (hosted-service.ts:1, multi-agent-postgres.ts:8 import 'memory-layer/...'; autonomous-agent.ts:42 nonexistent wrapped.trackedWorkItems); presets at src/core/presets.ts have no budget concept; README:13 still says "Version 4.0.0". Close-out identical to prior phases: triage reviews → Opus fix round → full local gate chain → commit → fetch-then-push → chase CI green.
...
> **STATUS: COMPLETE — COMMITTED + CI-GREEN on origin/main 2026-07-15** (commits afbb730 + 81b1814 + coverage ratchet; CI runs 29418915725→29419443126, all 5 jobs green on the second). Items 5.1–5.5 done, ships as 4.5.0. The debut of the full pg suite set immediately caught a REAL adapter-parity bug (compaction_log.model_call_made: memory defaulted true, sqlite false, pg NULL-crashed — now false everywhere with a conformance pin) plus a pg-pool double-close and a coverage-remapper crash on schema.sql — exactly the class of bug this phase exists to surface. Retrieval eval: 102 cases, in-run grep baseline provably fails the gate; cross-class redesigned after adversarial review proved a class-blind cosine ranker passed it (now r@1=0 for the cheat, 1.0 for the system). Memory-quality: raw actuals + knownWeak + ≥20 cases/metric; delta ratchet is the strict CI backstop. pg coverage measured 67.8/55.79/71.91/71.53 (stmts/branch/funcs/lines), thresholds ratcheted at measured−5. Adversarial reviewers caught: three Phase 2 suites' pg legs missing from the pg config (their first-ever CI execution is what caught the parity bug), blind debut thresholds, and the cross-class gap. Original staging note below.
>
> **STATUS: STAGED 2026-07-14 (superseded)** — executable workflow at `.claude/workflows/phase5-verification.mjs`. Launch with `Workflow({scriptPath: ".claude/workflows/phase5-verification.mjs"})`. Three stages: 4 parallel Opus workers on disjoint groups (RetrievalEval = ≥100-case categorized dataset with an in-run grep baseline the gate must beat; QualityDefit = raw-actuals reporting + knownWeak flags + ≥20 cases per metric + env-gated live-provider profile; PgBehavioral = delete regex pseudo-tests, vitest.postgres.config.ts for pg-scoped coverage in the postgres-integration job, seeded-PRNG replay-equivalence property test over sqlite+pg; Concurrency = pg-only race tier for claim/optimistic-lock/lazy-expiry/parallel-compaction) → 1 Wiring worker (sole owner of package.json + ci.yml; benchmark results tracking per 5.5, warn-only) → 2 adversarial reviewers (eval-rigor gaming; pg/concurrency correctness). Manager decisions pre-baked (D1–D7), notably: pg coverage runs in the postgres-integration job — the default test job has no pg, so removing the global exclusion there would measure zero (plan item 5.3 adjusted accordingly); thresholds are calibrate-then-ratchet, never aspirational; paraphrase honesty enforced programmatically (zero content-token overlap); benchmark comparison never fails CI. Standing lessons baked in: pg-gated suites must collect-as-skipped locally, ensurePgVectorExtension on bootstrap pools only, int4-safe timestamps. Ships as 4.5.0. Close-out per pattern: triage reviews → fix round → full gate chain → commit → push → chase CI (the postgres-integration job runs the equivalence + concurrency tiers with coverage for the first time — expect calibration findings there).
...
> **STATUS: COMPLETE — COMMITTED 2026-07-15, ships as 5.0.0.** Items 6.1–6.6 done via a five-stage sequential workflow (Duality∥ProviderErrors → Layering → Facades → Registry → Diet∥PyGen) + two adversarial reviewers. Delivered: AsyncStorageAdapter as a mapped type over StorageAdapter + loop-based wrapper (−463 duplicated lines, divergence is now a compile error); src/composition/ layer with dependency-cruiser boundary lint in CI (both contract type-cycles eliminated); six capability namespaces with all 93 flat methods as @deprecated shims (removal in 6.0.0), top level = 15 methods; an 87-entry operation registry driving HTTP dispatch and MCP curation by construction (core set 24 tools, full 62 behind --admin-tools/MEMORY_MCP_ADMIN_TOOLS; registry⇆spec parity structural); tiered exports + resolveEffectiveConfig() + docs/API_TIERS.md; typed ProviderUnavailableError at all 19 provider-boundary throws; python client generated from the registry (sync+async from one template, CI drift check, python:check green). SCOPE DECISIONS vs plan text: (D-REG) openapi.yaml stays authored — full YAML generation deferred (the minimal hand parser makes generated YAML riskier than the structural parity it buys); (6.5) only 1 export removal qualified under D-BREAK (estimateTokensLocal; everything else is used in-repo or legitimately public — Tier 3 documents 6.0.0 removal candidates instead). ACCEPTED FOLLOW-UPS (reviewer-noted, non-blocking): temporal/curation namespaces delegate over injected manager internals rather than fully extracted logic — substantive extraction is 6.0.0 work; the boundary allowlist (adapters/shared, sync-to-async, summarizers/prompts) can shrink by relocating those helpers into contracts.
